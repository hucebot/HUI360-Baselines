...
def mpjpe(predicted, target):
    ...
    """
    assert predicted.shape == target.shape, f"Expected shapes to be similar but got {predicted.shape} and {target.shape}"
...
def weighted_mpjpe(predicted, target, w):
    ...
    """
    assert predicted.shape == target.shape, f"Expected shapes to be similar but got {predicted.shape} and {target.shape}"
    assert w.shape[0] == predicted.shape[0], f"Expected weights shape 0 to be similar but got {w.shape} and {predicted.shape}"
...
def loss_angle_velocity(x, gt):
    ...
    """
    assert x.shape == gt.shape, f"Expected shapes to be similar but got {x.shape} and {gt.shape}"
    if x.shape[1]<=1:
        return torch.FloatTensor(1).fill_(0.)[0].to(x.device)
    x_a = get_angles(x)
    gt_a = get_angles(gt)
