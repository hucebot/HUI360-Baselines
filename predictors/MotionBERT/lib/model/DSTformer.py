...
class DSTformer(nn.Module):
    ...
    def forward(self, x, temp_embed_bounds = None):
        ret = None 
        ...
        # if provided, use the the temporal embeddings corresponding the segment of the input tensor
        if temp_embed_bounds is None:
            T_from = 0
            T_to = T
        else:
            T_from = temp_embed_bounds[0]
            T_to = temp_embed_bounds[1]
            
        x = x.reshape(-1, T, J, C) + self.temp_embed[:,T_from:T_to,:,:].contiguous()  # [B, T, J, dim_feat] + learnable temporal embeddings
...
class DSTDecodeOnly(nn.Module):
    def __init__(self, 
                 ...
                 orig_depth=5, 
                 norm_layer=nn.LayerNorm, 
                 input_type="representation", # "representation", "embeddings", "intermediates"
                 ):
        
        
        super().__init__()
        self.orig_depth = orig_depth
        self.dim_out = dim_out
        self.dim_rep = dim_rep
        self.dim_feat = dim_feat
    ...
    def forward(self, x):
        if self.input_type == "intermediates":
            B,T,J,DL = x.shape # [B, T, J, depth*dim_feat]
            D = DL // self.orig_depth
            assert(D == self.dim_feat), f"D={D} != dim_feat={self.dim_feat}"
            x = x.reshape(B, T, J, D, self.orig_depth)
            x = x[..., -1]  # last layer -> [B, T, J, dim_feat]
            x = einops.rearrange(x, 'B T J D -> (B T) J D') # [B*T, J, dim_feat]
            x = self.norm(x)                                        # [B*T, J, dim_feat]
            x = x.reshape(B, T, J, -1)                              # [B, T, J, dim_feat]
            x = self.pre_logits(x)                                  # [B, T, J, dim_rep]
            x = self.head(x)                                        # [B, T, J, dim_out]
        elif self.input_type == "embeddings":
            B,T,J,D = x.shape # [B, T, J, dim_feat]
            x = self.pre_logits(x)                                  # [B, T, J, dim_rep]
            x = self.head(x)                                        # [B, T, J, dim_out]
        elif self.input_type == "representation":
            B,T,J,D = x.shape # [B, T, J, dim_rep]
            x = self.head(x)                                        # [B, T, J, dim_out]
        return x