...
def get_track_input_possible_indices(track_data: pd.DataFrame, 
                                     ...
                                     center_on_onset: bool = False) -> tuple[int, list[tuple[int, int, int]]]:
    ...
    if first_interaction_index != -1:
        ...
        if center_on_onset:
            ...
            full_validity_mask[interaction_index_in_existence_mask+(input_length_in_frames//2)+1:] = False 
            full_validity_mask[:interaction_index_in_existence_mask-(input_length_in_frames//2)-1] = False 
        ...
        if ignore_negative_tracks_after_biggest_mask_size or force_aligment_with_biggest_mask_size:
            ...
            if center_on_onset:
                # for display purposes
                full_validity_mask[biggest_mask_index_in_existence_mask+(input_length_in_frames//2)+1:] = False
                full_validity_mask[:biggest_mask_index_in_existence_mask-(input_length_in_frames//2)-1] = False
                min_frames_in_segment = min_frames
            else:
                # normal behavior
                ...
                if force_aligment_with_biggest_mask_size:
                    # ignore_negative_tracks_after_biggest_mask_size and force_aligment_with_biggest_mask_size
                    min_frames_in_segment = min_frames + interaction_cutoff # we need to cut even before the first interaction
                else:
                    # ignore_negative_tracks_after_biggest_mask_size but not force_aligment_with_biggest_mask_size
                    min_frames_in_segment = min_frames
        else:
            # completetely random, normal behavior without ignore_negative_tracks_after_biggest_mask_size and without force_aligment_with_biggest_mask_size
            biggest_mask_index_in_existence_mask = None
            min_frames_in_segment = min_frames        
            ...
            if fixed_input_length:
                ...
                for possible_starting_point in possible_starting_points:
                    ...
                    if first_interaction_index != -1:
                        ...
                        if not center_on_onset and (force_aligment_with_biggest_mask_size and possible_starting_point + input_length_in_frames != biggest_mask_index_in_existence_mask - interaction_cutoff + 1):
                            # normal case when force_aligment_with_biggest_mask_size, force to end right on the biggest mask size
                            continue
                        else:
                            # either when not force_aligment_with_biggest_mask_size or for display purposes with center_on_onset
                            pass
                        label = 0
...
class HUIInteract360Interactive(Dataset):
    """ 
    This class serves as an alternative to the HUIInteract360 and is only dedicated to inference, such that we don't have 
    to respect the way we sampled at training time and can test using samples of tracks that include the interaction onset for example, 
    or that are longer than the actual model input length.
    """
